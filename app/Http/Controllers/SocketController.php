...
class SocketController extends Controller implements MessageComponentInterface
{
        ...
        if(isset($data->type))
        {
            ...
            if($data->type == 'request_connected_chat_user')
            {
                ...
                foreach($user_id_data as $user_id_row)
                {
                    ...
                    {
                        $last_seen = 'Был(а) в последний раз ' . date('H:i', strtotime($user_data->updated_at));
                    }
                    else
                    {
                        $last_seen = 'Был(а) в последний раз ' . date('d/m/Y H:i', strtotime($user_data->updated_at));
                    }
    ...
    public function onClose(ConnectionInterface $conn)
    {
        ...
        if(isset($queryarray['token']))
        {
            ...
            {
                $data['last_seen'] = 'Был(а) в последний раз ' . date('H:i');
            }
            else
            {
                $data['last_seen'] = 'Был(а) в последний раз ' . date('d/m/Y H:i');
            }
