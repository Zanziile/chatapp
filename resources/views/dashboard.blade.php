...
<div class="row">
	<div class="col-sm-4 col-lg-3">
		<div class="card">
			<div class="card-header"><b>Подключенные пользователи</b></div>
	...
	<div class="col-sm-4 col-lg-6">
		<div class="card">
			<div class="card-header">
				<div class="row">
					<div class="col col-md-6" id="chat_header"><b>Переписка с поьзователем</b></div>
	...
	<div class="col-sm-4 col-lg-3">
		<div class="card" style="height:255px; overflow-y: scroll;">
			<div class="card-header">
				<input type="text" class="form-control" placeholder="Искать пользователя..." autocomplete="off" id="search_people" onkeyup="search_user('{{ Auth::id() }}', this.value);" />
			</div>
			<div class="card-body">
		...
		<br />
		<div class="card" style="height:255px; overflow-y: scroll;">
			<div class="card-header"><b>Уведомления</b></div>
			<div class="card-body">
...
conn.onmessage = function(e){
    ...
    if (data.typing_status) {
        ...
        var typingElement = document.getElementById('typing' + from_user_id);

        if (typingElement) {
            typingElement.innerText = 'Печатает...';
        }
    }

    if (data.typing_status) {
        var userId = data.from_user_id;
        console.log('Пользователь с ID ' + from_user_id + ' печатает...');
    }
		...
		for(var count = 0; count < online_status_icon.length; count++)
		{
			if(online_status_icon[count].id == 'status_'+data.id)
			{
				if(data.status == 'Online')
				{
					...
					document.getElementById('last_seen_'+data.id+'').innerHTML = 'В сети';
				}
				else
				{
	...
	if(data.response_load_unconnected_user || data.response_search_user)
	{
		...
		if(data.data.length > 0)
		{
			...
			for(var count = 0; count < data.data.length; count++)
			{
				...
				{
					user_image = `<img src="{{ asset('images/no-image.png') }}" width="40" class="rounded-circle" />`
				}
	...
	if(data.response_load_notification)
	{
		...
		for(var count = 0; count < data.data.length; count++)
		{
			...
			if(data.data[count].notification_type == 'Send Request')
			{
				...
				{
					html += '<button type="button" name="send_request" class="btn btn-warning btn-sm float-end">Заявка отправлена</button>';
				}
				else
				{
					html += '<button type="button" name="send_request" class="btn btn-danger btn-sm float-end">Заявка отклонена</button>';
				}
			}
			else
			{
	...
	if(data.response_connected_chat_user)
	{
		...
		if(data.data.length > 0)
		{
			for(var count = 0; count < data.data.length; count++)
			{
				...
				{
					user_image = `<img src="{{ asset('images/no-image.png') }}" width="35" class="rounded-circle" />`;
				}
	...
	if(data.message)
	{
		...
		if(data.from_user_id == from_user_id)
		{
			...
			{
				icon_style = '<span id="chat_status_'+data.chat_message_id+'" class="float-end">Не доставлено</span>';
			}
			if(data.message_status == 'Send')
			{
				icon_style = '<span id="chat_status_'+data.chat_message_id+'" class="float-end">Доставлено</span>';
			}

			if(data.message_status == 'Read')
			{
				icon_style = '<span class="text-primary float-end" id="chat_status_'+data.chat_message_id+'">Прочтено</span>';
			}
	...
	if(data.chat_history)
	{
		...
		for(var count = 0; count < data.chat_history.length; count++)
		{
			if(data.chat_history[count].from_user_id == from_user_id)
			{
				...
				{
					icon_style = '<span id="chat_status_'+data.chat_history[count].id+'" class="float-end">Не доставлено</span>';
				}

				if(data.chat_history[count].message_status == 'Send')
				{
					icon_style = '<span id="chat_status_'+data.chat_history[count].id+'" class="float-end">Доставлено</span>';
				}

				if(data.chat_history[count].message_status == 'Read')
				{
					icon_style = '<span class="text-primary float-end" id="chat_status_'+data.chat_history[count].id+'">Прочтено</span>';
				}
	...
	if(data.update_message_status)
	{
		...
		if(chat_status_element)
		{
			...
			{
				chat_status_element.innerHTML = 'Прочтено';
			}
			if(data.update_message_status == 'Send')
			{
				chat_status_element.innerHTML = 'Доставлено';
			}
		}
...
function make_chat_area(user_id, to_user_name)
{
	...
	document.getElementById('chat_header').innerHTML = 'Общение с <b>'+to_user_name+'</b>';
...
function close_chat()
{
	document.getElementById('chat_header').innerHTML = 'Переписка';
...
function typing()
{
    document.getElementById('typing_status').innerHTML = html;
    var data = {
        event: 'typing',
        from_user_id: from_user_id,
    };
